...
Frame v0.1 validates the core macOS local screenshot loop: a menu bar app can be invoked by a global shortcut, let the user select a region, capture that region as PNG, then offer copy, save, or close through a minimal Quick Access panel.
...
- Quick Access panel with copy, save, and close.
...
- `SelectionOverlayController` creates one borderless overlay window per screen, tracks drag selection in global display coordinates, and reports the final selection.
...
- `QuickAccessPanelController` presents a small floating panel after capture and dispatches copy, save, and close commands.
...
7. User drags to select a region and releases.
8. Frame normalizes the selected region and captures PNG output.
9. Frame shows Quick Access near the captured region when possible.
10. User chooses copy, save, or close.
11. Copy writes the captured image to the system clipboard.
12. Save writes PNG data to Desktop with the default Frame filename.
13. Close dismisses the Quick Access panel without deleting saved or copied output.
...
- Selection overlay: dimmed transparent screen, visible selection rectangle, lightweight size readout if cheap to implement.
- Quick Access: compact floating panel with icon/text buttons for Copy, Save, and Close.
...
- If the selected rectangle is too small or empty, cancel capture without showing Quick Access.
...
- After granting permission and relaunching, drag a region and confirm Quick Access appears.
...
- Quick Access is small and dismissible and does not block normal system usage after close.
