...
- 不打开主窗口，常驻菜单栏。
- 默认使用 `Command+Shift+A` 开始区域截图；录屏快捷键默认未设置，可在设置中自定义。
- 支持多显示器。
...
- 选区录屏：设置录屏快捷键，或把选区 HUD 切换到录屏模式，选择 MP4 或 GIF，统一显示或隐藏鼠标指针与点击提示，并可在设置中自定义鼠标提示颜色；录入按住中的键盘提示，并从录屏 HUD 或红色菜单栏录制状态停止、重新开始或删除。
...
- 窗口截图样式：窗口截图可选择原图、柔和背景、画布光影或透明投影，默认是原图。
